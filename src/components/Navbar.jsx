...
const navClasses = "fixed top-9 left-0 right-0 z-50 flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-white/[0.08] bg-deep-hull/70 backdrop-blur-xl";
...
const Navbar = () => {
    ...
    const NavTag = mounted ? motion.nav : 'nav';

    return (
        <NavTag {...(mounted ? motionProps : {})} className={navClasses}>
            <Link href="/" className="flex items-center gap-2">
                <img src="/AhumAINoBg.png" alt="AhumAI Logo" className="w-7 h-7 sm:w-8 sm:h-8 object-contain" />
                <span className="text-base sm:text-lg font-bold tracking-tight text-white">AhumAI</span>
            </Link>

            {/* Kiro Powered Badge */}
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/[0.05] backdrop-blur-md border border-white/[0.08] hover:border-white/20 transition-colors duration-[0.15s] ease-out cursor-default group">
                ...
                <span className="text-xs font-medium text-fog-gray group-hover:text-warm-chalk transition-colors duration-[0.15s]">Kiro Powered</span>
            </div>
        </NavTag>
    );
};
